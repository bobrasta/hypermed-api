...
// New type for ServiceTicketController::overrideBilling() — finance needs to
// know when a CTO/Director corrects a ticket's warranty-vs-billable call,
// since it changes whether they should expect to invoice it.
return new class extends Migration
{
    ...
    {
        $prior = array_values(array_diff(self::TYPES, ['ticket_billing_overridden']));
        DB::statement('ALTER TABLE notifications DROP CONSTRAINT notifications_type_check');
        $list = "'" . implode("','", $prior) . "'";
        DB::statement("ALTER TABLE notifications ADD CONSTRAINT notifications_type_check CHECK (type IN ($list))");
    }
